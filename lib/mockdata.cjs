...
exports.pathOf = function pathOf(spec) {
	if (`${spec}`.startsWith('file://')) {
		return `${fs.realpathSync(`${url.fileURLToPath(spec)}`)}`;
	} else {
		return `${spec}`;
	}
};
...
function urlOf(spec, parents) {
	...
	for (const p of parents) {
		...
		if (fileURLExists(candidate)) {
			return `${url.pathToFileURL(fs.realpathSync(url.fileURLToPath(candidate)))}`;
		}
		for (const ext of ['.js', '.mjs', '.cjs', '.node', '/index.js', '/index.mjs', '/index.cjs', '/index.node']) {
			if (fileURLExists(`${candidate}${ext}`)) {
				return `${url.pathToFileURL(fs.realpathSync(url.fileURLToPath(`${candidate}${ext}`)))}`;
			}
		}
	}
	return spec;
